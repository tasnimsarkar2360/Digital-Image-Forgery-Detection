...
function result = infer_svm(image_path, threshold, checkpoint_path, show_figure, save_dir)
...
if nargin < 3 || isempty(checkpoint_path)
    checkpoint_path = fullfile(here, 'checkpoint', 'svm_model.mat');
end
...
if ~isfile(checkpoint_path)
    error('infer_svm:nocheckpoint', ...
        'No checkpoint found at %s - run train_svm first.', checkpoint_path);
end
...
[~, post] = predict(bundle.model, x_scaled);
prob_forged = double(post(2));
...
if ~isfolder(test_dir)
    error('infer_svm:notestdir', 'Test folder not found: %s', test_dir);
end
...
if isempty(files)
    error('infer_svm:notestimages', 'No images found in %s', test_dir);
end
...
if isempty(resp)
    error('infer_svm:badselection', 'No image selected.');
end
