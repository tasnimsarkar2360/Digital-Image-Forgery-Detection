function result = infer_logistic_regression(image_path, threshold, checkpoint_path, show_figure, save_dir)
...
if nargin < 3 || isempty(checkpoint_path)
    checkpoint_path = fullfile(here, 'checkpoint', 'logistic_regression_model.mat');
end
...
if ~isfile(checkpoint_path)
    error('infer_logistic_regression:nocheckpoint', ...
        'No checkpoint found at %s - run train_logistic_regression first.', checkpoint_path);
end
...
prob_forged = double(predict(bundle.model, x_scaled));
...
fprintf('%s\n', image_path);
fprintf('  model:        logistic_regression\n');
fprintf('  verdict:      %s\n', upper(verdict));
...
if ~isfolder(test_dir)
    error('infer_logistic_regression:notestdir', 'Test folder not found: %s', test_dir);
end
...
if isempty(files)
    error('infer_logistic_regression:notestimages', 'No images found in %s', test_dir);
end
...
if isempty(resp)
    error('infer_logistic_regression:badselection', 'No image selected.');
end
...
if isfile(candidate)
    ...
    error('infer_logistic_regression:nofile', ...
...
feats = struct( 'dct_benford_chi2', chi_sq, 'dct_benford_kl', kl_div,'dct_energy_mean', mean(energy_all), 'dct_energy_std', std(energy_all),'dct_energy_cv', std(energy_all) / (mean(energy_all) + 1e-6));
end
